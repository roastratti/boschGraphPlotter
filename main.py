...
def plot_graph(coordinates, line_names, xlabel, ylabel, title, show_values, cutin_jumpin_data, max_x_axis, max_y_axis):
    ...
    if show_jump_in:
        ax.plot([0, cut_in_x], [jump_in_y, jump_in_y], color='black', linewidth=0.8,linestyle="--")
        label_jump_in = f"Jump-in:\n {jump_in_y:.1f}"
        if jump_in_tol > 0:
    ...
    for i, line_data in enumerate(coordinates):
        ...
        if length != 0:
            ...
            offset_y = normal_y * offset + 0.01 * max_y_axis * i
        else:
            offset_x = offset_y = 0
    ...
    # MODIFIED: dashed lines go left/bottom for left side and right/bottom for right side
    for line_data in coordinates:
        for x, y in zip(line_data['X'], line_data['Y']):
            if x <= max_x_axis / 2:
                ...
                ax.plot([x, x], [0, y], 'k--', linewidth=0.8)
                ax.plot([x, max_x_axis], [y, y], 'k--', linewidth=0.8)
    ...
    if show_values:
        # MODIFIED: split left/right y-values based on X position
        left_y = []
        right_y = []
        for line_data in coordinates:
            for x, y, tol, br, show_y in zip(line_data['X'], line_data['Y'], line_data['Y Tol'], line_data['Y Brackets'], line_data['Show Y Val']):
                if show_y and y != jump_in_y:
                    label = format_val_label(y, tol, br)
                    if x <= max_x_axis / 2:
                        left_y.append((y, label))
                    else:
                        right_y.append((y, label))

        def place_y_labels(y_points, fig_x, align):
            ...
            for y_pos, label in y_stack:
                fig.text(fig_x, 0.18 + y_pos * 0.6, label,
                         ha=align, va='center', fontsize=9, color='black')

        place_y_labels(left_y, fig_x=0.15, align='right')
        place_y_labels(right_y, fig_x=0.80, align='left')
...
# Streamlit interface — NO CHANGES DONE BELOW
...
cutin_jumpin_df = pd.DataFrame({
    'X': [0.0], 'Y': [0.0],
    'X Tol': [0.0], 'Y Tol': [0.0],
    'X Brackets': [False], 'Y Brackets': [False],
    'Show X Val': [True], 'Show Y Val': [True]
})
...
for i in range(num_lines):
    ...
    df = pd.DataFrame({
        'X': [0.0] * num_coords, 'Y': [0.0] * num_coords,
        'X Tol': [0.0] * num_coords, 'Y Tol': [0.0] * num_coords,
        'X Brackets': [False] * num_coords, 'Y Brackets': [False] * num_coords,
        'Show X Val': [True] * num_coords, 'Show Y Val': [True] * num_coords,
    })
